...
#     # create kpts in meshgrid and resize them to image resolution
#     # 网格尺寸为 scale1, scale0
#     grid_offsets = torch.tensor([[0, 0], [7, 0], [0, 7], [7, 7]], device=device)  # [4, 2]
#
#     # 创建基础网格点（左上角）
#     grid_base1 = create_meshgrid(h1, w1, False, device).reshape(1, h1 * w1, 2).repeat(N, 1, 1)  # [N, hw, 2]
#     grid_base = scale1 * grid_base1
#
#     # 为每个基础点生成四个角点
#     grid_pt1_i = (grid_base.unsqueeze(2) + grid_offsets).reshape(N, h1 * w1 * 4, 2)  # [N, hw*4, 2]
#
#     # warp kpts bi-directionally and check reproj error
#     nonzero_m1, w_pt1_i = warp_kpts(grid_pt1_i, data['depth1'], data['depth0'], data['T_1to0'], data['K1'], data['K0'])
#     nonzero_m2, w_pt1_og = warp_kpts(w_pt1_i, data['depth0'], data['depth1'], data['T_0to1'], data['K0'], data['K1'])
...
#     mask_mutual = (dist < 1.5) & nonzero_m1 & nonzero_m2
#
#     # Step 2: 确保网格中的四个角点都有效，才保留该网格的四个角点
#     # 我们需要将每个网格四个角点的有效性条件检查作为一个整体
#     valid_grid_mask = mask_mutual.view(N, h1, w1, 4).all(dim=-1)  # [N, h1, w1]
#
#     # Step 3: 选择有效网格的四个角点
#     batched_corrs = []
#     for i in range(N):
#         # 选择当前批次有效的网格
#         valid_pts = valid_grid_mask[i].view(-1)  # [h1 * w1]
#         valid_pts_expanded = valid_pts.unsqueeze(1).repeat(1, 4).view(-1)  # [h1 * w1 * 4]
#         selected_pts = grid_pt1_i[i].view(-1, 2)[valid_pts_expanded]  # [有效网格的数目, 4, 2]
#         w_pts = w_pt1_i[i].view(-1, 2)[valid_pts_expanded]  # [有效网格的数目, 4, 2]
#
#         selected_pts_reshaped = selected_pts.view(-1, 4, 2)  # [有效网格数目, 4, 2]
#         w_pts_reshaped = w_pts.view(-1, 4, 2)  # [有效网格数目, 4, 2]
#
#         src_corner_points=w_pts_reshaped/ data['scale0'][i]
#         tgt_corner_points=selected_pts_reshaped / data['scale1'][i]
#
#         src_corner_points = src_corner_points / scale
#         tgt_corner_points = tgt_corner_points / scale
#
#         # 将源图像和目标图像的匹配点坐标连接起来
#         corr = torch.cat([src_corner_points,tgt_corner_points], dim=-1)  # [有效网格数目, 4, 4]
#
#         batched_corrs.append(corr)
#         # print(batched_corrs)
#         # for i in range(len(batched_corrs)):
#         #     plot_corrs(data['image0'][i], data['image1'][i], batched_corrs[i][:, :2]*8, batched_corrs[i][:, 2:]*8)
#
#     # 最终返回有效匹配点
#     return batched_corrs
...
def spvs_coarse(data, scale=8):
    ...
    # _, w_pt1_i = warp_kpts(grid_pt1_i, data['depth1'], data['depth0'], data['T_1to0'], data['K1'], data['K0'])
    batched_corrs = [torch.cat([w_pt1_i[i, mask_mutual[i]] / data['scale0'][i],
                                grid_pt1_i[i, mask_mutual[i]] / data['scale1'][i]], dim=-1) for i in
                     range(len(mask_mutual))]
...
# corrs 最终输出是一个包含有效网格角点的坐标列表，形状为 [batch_size, M, 4, 4] ，代表每个 batch 中有效网格的点坐标。
# [
#     Tensor of shape (5, 4, 4),  # 第一张图像中的匹配点
#     Tensor of shape (3, 4, 4)   # 第二张图像中的匹配点
# ]
